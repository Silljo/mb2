...
export class InteraktivnaMapaPage {
 ...
 loadMap() {
    ...
    this.map.one(GoogleMapsEvent.MAP_READY).then(() => {
        ...
        htmlInfoWindow.setContent('<div style="text-align: center;">Test</div>');


        // Now you can use all methods safely.
        //Dodavanje markera
        this.map.addMarker({
            icon: 'red', animation: 'BOUNCE',
            position: { lat: 46.00507, lng: 16.119108 } }).then(marker => {

            //Kad se klikne
            marker.on(GoogleMapsEvent.MARKER_CLICK).subscribe(() => {
                //Pomakni kameru rotiraj
                this.map.animateCamera({target: {lat: 46.00507, lng: 16.119106}, zoom: 17, tilt: 60, bearing: 140, duration: 3000});
